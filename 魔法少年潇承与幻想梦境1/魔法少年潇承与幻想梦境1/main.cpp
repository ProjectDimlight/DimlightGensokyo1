...
struct Shot
{
 ...
 bluefastchase, redslowstraight, blueslowstraight, yinyangShot, swordShot, explode, guiltyShot, FFFShot, excaliburShot;
list<Shot> friendly, aggresive;
...
WNDCLASSEX wcex;
HINSTANCE hInst;
HBITMAP bg, frame, title, bmp, optionimg[2], choosehero[2], gamehero[2], judgepoint,
		pausetitle, pauseoptionimg[2],
        wxcguard, wxcskill3, doomtag, doomimg,
		markup, markright, curname, bossname, resticon, winscreen, losescreen, restscreen;
HDC hdc, mdc, bufdc;
HWND hWnd;
ULONGLONG tPre, tNow, shotid;
int curstat, option, pauseoption, curhero, stage, heroSkillNameTimer, bossSkillNameTimer, shakeTimer, doomTimer;
...
int finalsparkDY(Shot *s)
{
	if (s->timer < 1)return 0;
	if (s->timer == 1)s->ty = hero.y - s->y;
	if (s->timer < 200)return hero.y - s->ty - s->y;
	s->uneraseable = false;
	return 1000;
}
...
int excaliburDX(Shot *s)
{
	if (s->timer < 50)
		if (sqr(s->x + s->jx - boss.x - (boss.w >> 1)) + sqr(s->y + s->jy - boss.y - (boss.h >> 1)) > sqr(60))
			return 10 * (boss.x + (boss.w >> 1) - s->x - s->jx) / (abs(boss.x + (boss.w >> 1) - s->x - s->jx) + abs(boss.y + (boss.h >> 1) - s->y - s->jy));
		else
			return 0;
	if (s->timer == 50)s->tx = 10 * (hero.x + (hero.w >> 1) - s->x) / (abs(hero.x + (hero.w >> 1) - s->x) + abs(hero.y + (hero.h >> 1) - s->y)) + rand() % 5 - 2;
	return s->tx;
}

int excaliburDY(Shot *s)
{
	if (s->timer < 50)
		if (sqr(s->x + s->jx - boss.x - (boss.w >> 1)) + sqr(s->y + s->jy - boss.y - (boss.h >> 1)) > sqr(60))
			return 10 * (boss.y + (boss.h >> 1) - s->y - s->jy) / (abs(boss.x + (boss.w >> 1) - s->x - s->jx) + abs(boss.y + (boss.h >> 1) - s->y - s->jy));
		else
			return 0;
	if (s->timer == 50)s->ty = 10 * (hero.y + (hero.h >> 1) - s->y) / (abs(hero.x + (hero.w >> 1) - s->x) + abs(hero.y + (hero.h >> 1) - s->y)) + rand() % 5 - 2;
	return s->ty;
}
...
void excaliburCast(Skill *s)
{
	...
	for (int i = 0; i < 300; i++)
	{
		Shot cur = excaliburShot;
		cur.id = shotid++;
		cur.x = boss.x + (boss.w >> 1) + (rand() % 401 - 200);
		cur.y = boss.y + (boss.h >> 1) + (rand() % 401 - 200);
		aggresive.push_back(cur);
	}
}

void excaliburProc(Skill *s)
{
	if (s->timer == 50)
	{
		shakeTimer = 100;
...
{
	list<Shot>tmp = aggresive; aggresive.clear();
	
	int x2 = hero.x + (hero.w >> 1), y2 = hero.y + (hero.h >> 1);
	for (list<Shot>::iterator i = tmp.begin(); i != tmp.end(); i++)
	{
		Shot cur = *i;
		...
		{
			hero.s1.curcd = max(hero.s1.curcd - 7, 0);
			hero.s2.curcd = max(hero.s2.curcd - 7, 0);
			hero.s3.curcd = max(hero.s3.curcd - 7, 0);
		}
	}
...
void initShot()
{
	...
	guiltyShot.hurt = 10000;
	guiltyShot.jx = 15;
	guiltyShot.jy = 15;
	guiltyShot.w = 30;
	guiltyShot.h = 30;
	guiltyShot.r = 15;
	guiltyShot.dx = guiltyDX;
	guiltyShot.dy = guiltyDY;
	guiltyShot.unbreakable = false;
	guiltyShot.uneraseable = false;
	guiltyShot.hitboss = false;
	guiltyShot.cnt = 1;
	guiltyShot.img[0] = (HBITMAP)LoadImage(NULL, L"..\\pictures\\guiltyshot.bmp", IMAGE_BITMAP, 30, 30, LR_LOADFROMFILE);
	...
	FFFShot.dx = FFFDX;
	FFFShot.dy = FFFDY;
	FFFShot.unbreakable = false;
	FFFShot.uneraseable = false;
	FFFShot.hitboss = false;
	FFFShot.cnt = 1;
	FFFShot.img[0] = (HBITMAP)LoadImage(NULL, L"..\\pictures\\FFFshot.bmp", IMAGE_BITMAP, 30, 30, LR_LOADFROMFILE);

	excaliburShot.hurt = 10000;
	excaliburShot.jx = 5;
	excaliburShot.jy = 5;
	excaliburShot.w = 11;
	excaliburShot.h = 11;
	excaliburShot.r = 6;
	excaliburShot.dx = excaliburDX;
	excaliburShot.dy = excaliburDY;
	excaliburShot.unbreakable = false;
	excaliburShot.uneraseable = false;
	excaliburShot.hitboss = false;
	excaliburShot.cnt = 1;
	excaliburShot.img[0] = (HBITMAP)LoadImage(NULL, L"..\\pictures\\yellowsmallshot.bmp", IMAGE_BITMAP, 11, 11, LR_LOADFROMFILE);
}
...
void initSkill()
{
	...
	army.cost = 0;
	army.timer = -1;
	army.castfunc = armyCast;
	army.procfunc = armyProc;
	army.name = (HBITMAP)LoadImage(NULL, L"..\\pictures\\army.bmp", IMAGE_BITMAP, 480, 24, LR_LOADFROMFILE);
	...
	excalibur.cost = 0;
	excalibur.timer = -1;
	excalibur.castfunc = excaliburCast;
	excalibur.procfunc = excaliburProc;
	excalibur.name = (HBITMAP)LoadImage(NULL, L"..\\pictures\\excalibur.bmp", IMAGE_BITMAP, 480, 24, LR_LOADFROMFILE);
}
...
void initBoss()
{
	...
	bosses[2].blood[1] = bosses[2].bloodlimit[1] = 10000;
	bosses[2].blood[2] = bosses[2].bloodlimit[2] = 15000;
	bosses[2].blood[3] = bosses[2].bloodlimit[3] = 15000;
	bosses[2].s[0] = swords;
	bosses[2].s[1] = charge;
	bosses[2].s[2] = army;
	bosses[2].s[3] = excalibur;
	bosses[2].tot = 4;
	bosses[2].cur = 0;
...
void loadImage()
{
	...
	pauseoptionimg[0] = (HBITMAP)LoadImage(NULL, L"..\\pictures\\continue.bmp", IMAGE_BITMAP, 150, 30, LR_LOADFROMFILE);
	pauseoptionimg[1] = (HBITMAP)LoadImage(NULL, L"..\\pictures\\exit.bmp", IMAGE_BITMAP, 96, 30, LR_LOADFROMFILE);
	pausetitle = (HBITMAP)LoadImage(NULL, L"..\\pictures\\pause.bmp", IMAGE_BITMAP, 426, 70, LR_LOADFROMFILE);
	...
	judgepoint = (HBITMAP)LoadImage(NULL, L"..\\pictures\\judge.bmp", IMAGE_BITMAP, 8, 8, LR_LOADFROMFILE);
...
void loadStage(int s)
{
	...
	hero.s1.curcd = hero.s2.curcd = hero.s3.curcd = 0;
...
void buildChoose()
{
	...
	Transparent(mdc, 160 + curhero * 275, 420, 30, 30, bufdc, 0, 0, 30, 30, RGB(0, 0, 0));
}
...
void buildGame()
{
	...
	for (list<Shot>::iterator i = aggresive.begin(); i != aggresive.end(); i++)
	{
		SelectObject(bufdc, i->img[i->timer/5 % i->cnt]);
		Transparent(mdc, i->x + dx, i->y + dy, i->w, i->h, bufdc, 0, 0, i->w, i->h, RGB(0, 0, 0));
	}

	for (list<Shot>::iterator i = friendly.begin(); i != friendly.end(); i++)
	{
		SelectObject(bufdc, i->img[i->timer/5 % i->cnt]);
		Transparent(mdc, i->x + dx, i->y + dy, i->w, i->h, bufdc, 0, 0, i->w, i->h, RGB(0, 0, 0));
	}
	...
	if (hero.ms <= 6)
	{
		SelectObject(bufdc, judgepoint);
		Transparent(mdc, hero.x + (hero.w >> 1) - 4 + dx, hero.y + (hero.h >> 1) - 4 + dy, 8, 8, bufdc, 0, 0, 8, 8, RGB(0, 0, 0));
	}
...
void buildPause()
{
	...
	SelectObject(bufdc, pausetitle);
	Transparent(mdc,   0, 110, 426, 70, bufdc, 0, 0, 426, 70, RGB(0, 0, 0));

	SelectObject(bufdc, pauseoptionimg[0]);
	Transparent(mdc, 150, 300, 150, 30, bufdc, 0, 0, 150, 30, RGB(0, 0, 0));
	SelectObject(bufdc, pauseoptionimg[1]);
	Transparent(mdc, 150, 340, 96, 30, bufdc, 0, 0, 96, 30, RGB(0, 0, 0));

	SelectObject(bufdc, markright);
	Transparent(mdc, 125, 305 + pauseoption * 40, 20, 20, bufdc, 0, 0, 30, 30, RGB(0, 0, 0));
}

void welcomeProc()
{
	if (option == 0)
		curstat = CHOOSE, curhero = 0, stage = 0;
	else
...
void timerProc()
{
	...
	//shot move
	list<Shot> tmp = friendly;friendly.clear();
	bool shutdown = false;
	for (list<Shot>::iterator i = tmp.begin(); i != tmp.end(); i++)
	{
		Shot cur = *i;
		...
		if (cur.clearagg)
		{
			list<Shot> tmp2 = aggresive; aggresive.clear();
			for (list<Shot>::iterator j = tmp2.begin(); j != tmp2.end(); j++)
			{
				Shot cur2 = *j;
				x2 = cur2.x + cur2.jx, y2 = cur2.y + cur2.jy;
				if (sqr(x1 - x2) + sqr(y1 - y2) > sqr(cur.r + cur2.r))
					aggresive.push_back(cur2);
			}
		}
	...
	else
	{
		...
		case WM_KEYDOWN:
			...
			else if (curstat == CHOOSE)
			{
				...
				{
					curhero++;
					if (curhero == 2) curhero = 0;
				}
				else if (KEYDOWN(VK_LEFT))
				{
					curhero--;
					if (curhero == -1) curhero = 1;
				}
				else if (KEYDOWN('Z'))
				{
					chooseProc();
			...
			else if (curstat == PAUSE)
			{
				...
				{
					pauseoption--;
					if (pauseoption == -1) pauseoption = 1;
				}else if (KEYDOWN('Z'))
				{
					pauseProc();
				}
